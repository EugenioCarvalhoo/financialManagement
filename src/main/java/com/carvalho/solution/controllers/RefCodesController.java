...
public class RefCodesController {
    ...
    @PostMapping()
    public ResponseEntity<RefCodesDTO> save(@RequestBody RefCodesDTO dto) { 
        RefCodesDTO result = refCodesService.save(dto, UserContext.getTenantId());
        return ResponseEntity.ok().body(result);  
    }

    @PutMapping("{id}")
    public ResponseEntity<RefCodesDTO> update(@PathVariable String id, @RequestBody RefCodesDTO dto) {
